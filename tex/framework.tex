...
solo hilo al comienzo de se ejecución y durante sus secciones secuenciales, al hilo
inicial se lo llama hilo principal. Cuando se encuentra una zona paralela de $N$
tareas, el hilo principal crea $N-1$ nuevos hilos. Cada hilo (incluido el hilo
principal) ejecuta una de las $N$. Esta etapa es llamada \emph{fork}. El hilo
...
datos se realiza a través de la directiva \emph{parallel for}, mientras que la
...
el \emph{fork}, sino que se mantienen inactivos hasta que se encuentra una
sección paralela. Por otro lado, si la sección paralela consta de gran cantidad
de tareas, se puede crear un numero limitado de hilos y crear una cola de
tareas. Cada hilo ejecuta una tarea de la cola, y cuando termina de ejecutar esa
tarea toma otra de la cola. La ejecución secuencial no continua hasta que se
terminen de ejecutar todas las tareas de la sección paralela.

WIP:

El segundo modelo que implementado en \emph{OpenMP} a partir de la versión $3.0$
es el de tareas. En este modelo el hilo principal crea una cantidad fija de
hilos de ejecución para la ejecución de tareas. Cuando se encuentra una sección
que puede ser ejecutada paralelamente al hilo principal, se crea una tarea. Esta
tarea sera ejecutada por los hilos de ejecución creados por el hilo principal,
en caso de que no haya hilos de ejecución libres para ejecutar la tarea, esta se
coloca en una cola de espera. Esto permite crear tareas de forma distribuida en
el tiempo, y facilita el control de la cantidad de hilos de ejecución creados.
